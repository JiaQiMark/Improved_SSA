    ...
    for t in range(max_iterations):
        ...
        for i in range(population):
            inertia_weight[i] = weight_min + (weight_max - weight_min) / 2 * (math.sin(
                math.pi * ((fitness[i] - best_finess) / (worst_fitness - best_finess + 1e-20)) + math.pi/2) + 1)
        ...
        for i in range(producer_num):
            ...
            if R2 < ST:
                ...
                position[p_i, :] = inertia_weight[i] * position[p_i, :] + q * l_dim * cauchy_gauss
        ...
        for i in range(0, population - producer_num):
            ...
            if o_i > (population / 2):
                q = np.random.normal(0, 1 , 1)
                # position[s_i, :] = inverted_weight[s_i] * q * np.exp((worst_positon - position[s_i, :])/(o_i**2))
                # position[s_i, :] = inverted_weight[s_i] * (worst_positon - position[s_i, :]) / 2
                position[s_i, :] = (inertia_weight[s_i] * position[s_i, :] +
                                    inverted_weight[s_i] * (best_position - position[s_i, :]) / 2)
            else:
                l_dim = np.ones(dim)
                a = np.floor(np.random.rand(1, dim) * 2) * 2 - 1
                a_plus = 1 / (a.T * np.dot(a, a.T))
                position[s_i, :] = (next_best_position + inverted_weight[s_i] * l_dim *
                                    np.dot(np.abs(position[s_i, :] - next_best_position), a_plus))
        ...
        for i in range(num_alerter):
            ...
            if f_i > f_g:
                ...
                position[a_i, :] = best_position + inverted_weight[a_i] * beta * np.abs(position[a_i, :] - best_position)
            elif f_i == f_g:
                e = 1e-20
                k = np.random.uniform(-1, 1, 1)
                position[a_i, :] = (inertia_weight[a_i] * position[a_i, :] +
